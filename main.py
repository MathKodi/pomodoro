from tkinter import *
import math
# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
timer = None
# ---------------------------- TIMER RESET ------------------------------- # 

def restart():
    window.after_cancel(timer)
    canvas.itemconfig(timertxt, text="00:00")
    title.config(text="Timer", fg=GREEN)
    checkmark.config(text="")
    btnS["state"] = "active"
    global reps
    reps = 0
# ---------------------------- TIMER MECHANISM ------------------------------- # 

def start_timer():
    global reps
    reps += 1
    work_sec = WORK_MIN * 60
    short_break_sec = SHORT_BREAK_MIN * 60
    long_break_sec = LONG_BREAK_MIN * 60

    if reps % 8 == 0:
        title.config(text="Break Time", fg=RED)
        count_down(long_break_sec)
    elif reps % 2 == 0:
        title.config(text="Break Time", fg=PINK)
        count_down(short_break_sec)
    else:
        title.config(text="Work Time", fg=GREEN)
        count_down(work_sec)


# ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 
def count_down(count):
    btnS["state"] = "disabled"
    min = math.floor(count/60)
    sec = count % 60

    if sec < 10:
        sec = f"0{count}"

    canvas.itemconfig(timertxt, text=f"{min}:{sec}")
    if count > 0:
        global timer
        timer = window.after(1000, count_down, count - 1)
    else:
        start_timer()
        mark = ""
        work_sessions = math.floor(reps/2)
        for _ in range(work_sessions):
            mark += "✔"
        checkmark.config(text=mark)
# ---------------------------- UI SETUP ------------------------------- #
window = Tk()
window.title("Pomodoro")
window.config(padx=100, pady=50, bg=YELLOW)

checkmark = Label(fg=GREEN, font=(FONT_NAME, 14), bg=YELLOW)
checkmark.grid(column=1,row=3)

btnS = Button(text="Start", font=FONT_NAME, bg=YELLOW, borderwidth=1, command=start_timer)
btnS.grid(column=0, row=2)

btnR = Button(text="Restart", font=FONT_NAME, bg=YELLOW, borderwidth=1, command=restart)
btnR.grid(column=2, row=2)

title = Label(text="Timer", fg=GREEN, font=(FONT_NAME, 50, "bold"), bg=YELLOW)
title.grid(column=1, row=0)

canvas = Canvas(width=200, height=224, bg=YELLOW, highlightthickness=0)
img = PhotoImage(file="tomato.png")
canvas.create_image(100, 112, image=img)
timertxt = canvas.create_text(100, 130, text="00:00", fill="white", font=(FONT_NAME, 35, "bold"))
canvas.grid(column=1, row=1)

window.mainloop()